...
2. **Vercel:** Import this repo. Either leave **Root Directory** empty: root **`vercel.json`** runs `npm install` (root declares `next` for Vercel’s detector) **and** `npm install --prefix whitelane-next`, then `npm run build --prefix whitelane-next`. **Or** set **Root Directory** to **`whitelane-next`**. Clear any stale **Output Directory** = `dist` from old Vite/Laravel settings.
